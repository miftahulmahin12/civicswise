...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-surface p-6 shadow-[var(--shadow-raised)] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-1.5 text-ink-faint transition-colors hover:bg-paper-dim hover:text-ink">
        <X className="h-5 w-5" />
